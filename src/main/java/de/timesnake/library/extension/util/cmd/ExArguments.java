  ...
  public ExArguments(Sender sender, String[] args, boolean allowDuplicateOptions) {
    ...
    for (String arg : args) {
      if (arg.startsWith("--")) {
        ...
        if (name.isEmpty()) {
          throw new ArgumentParseException("Error while parsing option, no identifier found",
              arg.indexOf('='));
        }

        String value = argWithValue[1];
        if (value.isEmpty()) {
          throw new ArgumentParseException("Error while parsing option, no value found",
              arg.indexOf('='));
        }
        ...
        if (argWithValue.length == 1) {
          ...
          throw new ArgumentParseException("Error while parsing options, to many values",
              arg.lastIndexOf('='));
        }
      } else {
